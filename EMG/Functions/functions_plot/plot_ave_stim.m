...
destPath = [TestPath1,pathCompSep,'Results',pathCompSep,'EMG',pathCompSep,(LEG{leg}),pathCompSep,'stim',pathCompSep,'average',pathCompSep,'FiringPattern'];

if ~exist(destPath,'dir')
    mkdir(destPath)
end

  size_of_muscle =  numel(fieldnames(emgconcat.emgconcat.stim.(LEG{leg}).average.seg));
  name_of_muscle = fieldnames(emgconcat.emgconcat.stim.(LEG{leg}).average.seg);

for musclesize = 1:size_of_muscle

    name_of_condition = fieldnames(emgconcat.emgconcat.stim.(LEG{leg}).average.seg.(name_of_muscle{musclesize,1}));
    size_of_condition = numel(fieldnames(emgconcat.emgconcat.stim.(LEG{leg}).average.seg.(name_of_muscle{musclesize,1})));

    for conditionsize = 1: size_of_condition

        y  = emgconcat.emgconcat.stim.(LEG{leg}).average.seg.(name_of_muscle{musclesize,1}).(name_of_condition{conditionsize,1})';
        figure(conditionsize);
        spm1d.plot.plot_meanSD_seg(y',(name_of_muscle{musclesize,1}),'color',[0, 0.4470, 0.7410]);
        titlename = strcat("Average - ","FP"," - ", (LEG{leg})," - ",(name_of_muscle{musclesize,1}),"-", (name_of_condition{conditionsize,1}),"- stim");
        title(titlename)
