...
function plot_ave_gait_xstim_total(emgconcat,LEG,leg,TestPath1,pathCompSep)
...
for musclesize = 1:size_of_muscle
    ...
    h=figure;
    titlename = strcat((LEG{leg})," - ",(name_of_muscle{musclesize,1}),"Condition Comparison - ", "-xstim");
    title(titlename)
    ...
    titlename = strcat((LEG{leg})," - ",(name_of_muscle{musclesize,1}),"Condition Comparison - 2 ", "-xstim");
    title(titlename)
    saveas(h, fullfile(destPath, titlename), 'pdf');
