...
markerlist = generateMarkerList(VD); % look for available markers

fileidtoignore = [];
fileswithmissingMarkers = [];
nCriticalMarkers = length(markerlist);
for i = 1:nCriticalMarkers
    if isfield(VD, (markerlist{i}))
        ...
        for dirtn = 1:3
            if dirtn == 1
                timeSeriesx = VD.(markerlist{i})(:,1);
            elseif dirtn == 2
                timeSeriesx = VD.(markerlist{i})(:,2);
            elseif dirtn == 3
                timeSeriesx = VD.(markerlist{i})(:,3);
            end
            ...
            if nooflargeholes > 0
                ...
                interpolatedVD.(genvarname(markerlist{i})).Values.x_coord = VD.(genvarname(markerlist{i})).Values.x_coord;
                interpolatedVD.(genvarname(markerlist{i})).Values.y_coord = VD.(genvarname(markerlist{i})).Values.y_coord;
                interpolatedVD.(genvarname(markerlist{i})).Values.z_coord = VD.(genvarname(markerlist{i})).Values.z_coord;
