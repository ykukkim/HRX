function [LmidStlocs, RmidStlocs, d, mean_d] = findmidstance2(...
...
[~, midStCandlocs] = findlocalmax(d, round(mpd/2), 'min');
...
Lset = [];
% for k = 1:2:size(Left_matrix)-1
%     Lset = union(Lset,Left_matrix(k,2):Left_matrix(k+1,2));
% end
...
% Rset = [];
% for k = 1:2:size(Right_matrix)-1
%     Rset = union(Rset,Right_matrix(k,2):Right_matrix(k+1,2));
% end
